...
// This signature MUST match exactly what MPI expects:
// void *invec (the incoming data from another process)
// void *inoutvec (the running total / current data)
// int *len (how many elements are being reduced at once)
// MPI_Datatype *datatype (the type of the data)
// -------------------------------------------------------------------------
void my_custom_sum(void *invec, void *inoutvec, int *len, MPI_Datatype *datatype) {
    ...
    // Even if we only send 1 item, MPI expects this to handle arrays
    for (int i = 0; i < *len; i++) {
...
int main(int argc, char **argv) {
    ...
    // Distribute and compute locally
    ...
    // -------------------------------------------------------------------------
    // TEST 2: CUSTOM MPI OPERATION
    // -------------------------------------------------------------------------
    long long global_sum_custom = 0;
    MPI_Op MY_MPI_CUSTOM_SUM; // Declare the operation handle

    // Register the function. The '1' means it is commutative (A+B = B+A)
    MPI_Op_create(&my_custom_sum, 1, &MY_MPI_CUSTOM_SUM);

    MPI_Barrier(MPI_COMM_WORLD);
    double start_custom = MPI_Wtime();
    
    // Notice we pass MY_MPI_CUSTOM_SUM instead of MPI_SUM
    MPI_Reduce(&local_sum, &global_sum_custom, 1, MPI_LONG_LONG, MY_MPI_CUSTOM_SUM, 0, MPI_COMM_WORLD);
    
    double end_custom = MPI_Wtime();

    // Free the custom operation to prevent memory leaks
